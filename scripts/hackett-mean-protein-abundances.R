...
# summarized_abundances_across_conditions %>% 
#   inner_join(yeast_lengths %>% dplyr::rename(protein_name = id), 
#              by = 'protein_name') %>% 
#   ggplot(aes(x = mean_prot_across_conditions*lenghts, y = mean_prot_mf)) +
#   geom_point(aes(size = lenghts)) +
#   geom_label(aes(label = protein_name)) +
#   scale_y_log10() +
#   scale_x_log10() #+
...
# summarized_abundances_across_conditions %>% 
#   
#   dplyr::filter(protein_name == "YCR012W")
#   
# summarized_abundances_across_conditions %>% 
#   dplyr::rename(Gene = protein_name) %>% 
#   inner_join(ho_proteins, by = 'Gene') %>% 
#   ggplot(aes(y = mean_prot_mf, x = med_mol_cell)) +
#   geom_point() +
#   geom_errorbar(aes(ymin = mean_prot_mf - sd_prot_mf, ymax = mean_prot_mf + sd_prot_mf)) +
#   scale_y_log10() +
#   scale_x_log10() 
...
# summarized_abundances_across_conditions %>% 
#   dplyr::rename(Gene = protein_name) %>% 
#   inner_join(ho_proteins %>% dplyr::select(Gene, med_mol_cell), 
#              by = "Gene") %>% 
#   ggplot(aes(x = med_mol_cell, y = mean_prot_across_conditions)) +
#   geom_point() +
#   scale_x_log10() +
#   scale_y_log10()

# peptide_intermediate_list <- c()
# for(i in 1:length(peptides_tester)){
#   removed_last_two <- substr(peptides_tester[i], start = 1,
#                              stop = nchar(peptides_tester[i]) - 2)
#   peptide_intermediate_list <- c(peptide_intermediate_list, removed_last_two)
# }
# sum_test <- c()
# unique_peptides <- unique(peptide_intermediate_list)
# for(j in 1:length(unique_peptides)){
#   sum_test <- c(sum_test, nchar(unique_peptides[j]))
# }

# ho_proteins %>% dplyr::filter(Gene == 'YCR012W') %>% as.data.frame()
# ho_proteins$med_mol_cell %>% sum(na.rm = TRUE)
