...
gsh2_plot <- joined_df_flux_prot_mean_no_negative %>% 
  ...
  annotate(geom = 'text', x = -0.65, 2, 
           # p + annotate("text", x = 4, y = 30, label = expression(rho), size = 5, color = "red")
...
coefficient_gamma_glut <- gamma_glut_coefs %>% 
  ...
  # xlab(expression(paste("Coefficient Value (mmol per hour per mL)/(", 
  #                       "Protein abundance standard deviation)^{-1}"))) +  # coord_flip() +
  # labs(y = 'Observed Rate of GSH Synthesis<br>(nmol hour<sup>\u22121</sup> mL<sup>\u22121</sup>)',
  #      x = 'Predicted Rate of GSH Synthesis<br>(nmol hour<sup>\u22121</sup> mL<sup>\u22121</sup>)') +
  # theme(
  #   axis.title = ggtext::element_markdown()
  # ) +
  # labs(x = expression(paste("Coefficient Value (mmol per hour per mL)",~(Protein~abundance~standard~deviation)^2))
  # labs(x = expression("Coefficient Value (mmol per hour per mL)" %*% ("Log(Protein abundance) standard deviation")^-1)) +
...
# coefficient_gamma_glut_raw <- gamma_glut_coefs %>% 
#   dplyr::filter(long_names != '(Intercept)') %>% 
#   # group_by(coefficient_names, long_names) %>% 
#   # summarize(mean_coef_value = mean(coefficient_values),
#             # sd_coef_value = sd(coefficient_values)) %>% 
#   dplyr::mutate(new_name = paste(long_names, ' (', coefficient_names, ')', sep = '')) %>%
#   ggplot(aes(x = coefficient_values, 
#              y = fct_reorder(new_name, coefficient_values, .desc = FALSE))) +
#   geom_point(size = 3) +
#   ylab('Protein') +
#   ggtitle('D. Ridge regression coefficients') +
...
#         axis.title = element_text(size = 13),
#         axis.text = element_text(size = 13)) +
#   scale_colour_manual(values = c(rep('black', 8), 'coral3', rep('black', 8))) +
#   # theme(axis.text.x = element_text(hjust = 1, angle = 60)) +
...
# # recoding the glmnet approach --------------------------------------------
# 
# glut_out_ridge <- cv.glmnet(x = gsh_synth_scaled %>% 
#                               dplyr::select(-exp_condition, 
#                                             -flux, 
#                                             -growth_rate, 
#                                             -nutrient) %>% as.matrix(),
#        y = gsh_synth_scaled$flux, alpha = 0)
# 
# glut_out_ridge$lambda.min
# 
# coef(glut_out_ridge)
# 
# YIL010W exclusion switches YCR083W from positive to negative
# 
# 
# 
